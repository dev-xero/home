...
summary: JSON is cool, I've always wondered how parsers read json files and convert them into native data structures, so I thought to myself, why don't I try building one from the ground up?
...
So, even if you don't like Rust (like @anu ¬‿¬), you can always follow along in your fav programming language.
...
This is my ouput! Working perfectly so far. Let's get started with the tokenizer.
...
Our token struct is composed of two parts, a “kind” and an optional “value”. The kind of a token can be any of: string, number, boolean, left brace, right brace, etc as per the official JSON specs. Here's a list of all the tokens our parser will handle.
...
So each token will have the `kind` attribute. Some tokens can have a `value`, like strings and numbers. Everything else won't.
