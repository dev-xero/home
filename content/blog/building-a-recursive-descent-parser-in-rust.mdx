...
The check in the last few lines ensure that our string ends with a proper alphanumeric character or brace.
...
So, what have we here? The string scanning works by keeping track of the start position (right after the quotes) and the possible right before the ending quote.
