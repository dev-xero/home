...
summary: JSON is cool, but parsers are cooler, so why don't we try building a JSON parser from scratch!
...
_This post is long and rather technical, you might wanna grab a coffee for this_
...
So, even if you don't like Rust (like @anu lol), you can always reproduce this in your fav programming language (bonus points if you chose C).
...
<RoundedImage
    ...
    alt="diagram-1" context="This is what we're working with"
/>
...
Looks good. In the mean time, let's add a catch-all to panic whenever we encounter unknown tokens:
...
What this code does is checking whether the current lexeme is a number, i.e. between 0 to 9, or starts with a minus sign. If it's a minus sign, we consume it and move forward. Since we're in numbers domain now, we'll delege scanning the number to another function: `scan_num()`, then push the result to our token vector.
...
The logic for scanning identifiers is simple: We know an identifier won't exceed 5 characters, so I'll move the pointer 4 characters ahead to see if it matches "true" or "null" then return the appropriate string.

Else, check the next five characters if it matches "false" and return that instead. If we have whatever garbage value that isn't **true**, **false** or **null**, panic on execution.

_Panics_ in Rust are similar to throwing errors in other programming languages.

We're really close to finishing our tokenizer, all that's left is to use the `scan_identifier()` function.
...
We swtich to identifier scanning mode when we encounter an alphabetic character that isn't one of our earlier matched expressions, and push their tokens to our vector.
...
Defining our grammar in this way ensures we can create recursive functions that call themselves in just the right way to build a complex, yet effective heirarchy of tokens.
